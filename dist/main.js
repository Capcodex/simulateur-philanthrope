...
const profils = {
    "stratège": {
        ...
        structureDetail: "Le fonds de dotation est un outil efficace pour financer durablement vos causes d’intérêt général, tout en conservant un haut niveau de discrétion et de simplicité administrative. Il est idéal pour un engagement structuré, sans surmédiatisation."
    },
    "mecene": {
        ...
        structureDetail: "La fondation abritée vous permet de concentrer vos moyens dans un cadre sécurisé et professionnel, sans en gérer la lourdeur administrative. Elle offre visibilité et reconnaissance, tout en vous laissant une grande liberté dans le choix des projets."
    },
    "activiste": {
        ...
        structureDetail: "L’association ou la structure agile (type fonds de soutien ou coopérative) est parfaitement adaptée à un engagement rapide, visible et opérationnel. Elle vous donne la flexibilité nécessaire pour agir, mobiliser et communiquer efficacement."
    },
    "familial": {
        ...
        structureDetail: "La fondation familiale vous permet de structurer votre philanthropie sur plusieurs générations. Quant à la donation temporaire d’usufruit, elle combine impact immédiat et optimisation fiscale, dans une logique de don progressif."
    },
};
function identifierProfil(finalite, visibilite, temps, domaine) {
    if (finalite === "societe" && temps === "long" && visibilite === "anonyme")
        return profils["stratège"];
    if (domaine === "culture" && finalite === "emotion" && visibilite === "maitrise")
        return profils["mecene"];
    if (domaine === "social" || domaine === "environnement")
        return profils["activiste"];
    if (finalite === "famille" || domaine === "jeunesse")
        return profils["familial"];
    return profils["stratège"];
}
document.addEventListener('DOMContentLoaded', () => {
    const form = document.getElementById('philanthrope-form');
    const resultat = document.getElementById('resultat');
    form.addEventListener('submit', (e) => {
        e.preventDefault();
        const finalite = document.getElementById('finalite').value;
